...
## Model Empirical Evaluation (Feb 2025)

Evaluated on GW17-26 of 2025-2026 season (~6,700 prediction-actual pairs). Models were trained on all available data — these numbers include some in-sample data, but the walk-forward MAE during training (truly out-of-sample) shows similar results.

### Overall Accuracy

| Metric | Our Model | FPL ep_next |
|--------|-----------|-------------|
| MAE | **0.986** | 1.193 |
| Spearman | **0.742** | 0.703 |
| Per-GW MAE wins | **10/10** | 0/10 |

Beats FPL's own `ep_next` prediction in every single gameweek.

### Captain Picks

| | Our Model | ep_next | Best Possible |
|--|-----------|---------|---------------|
| Avg captain pts | **12.6** | 3.3 | 16.8 |
| Capture rate | **75%** | 20% | 100% |
| Nailed exact best | 3/10 | 0/10 | — |

Captain picking is the model's strongest feature. The `captain_score` blend (0.4×mean + 0.6×Q80) correctly identifies explosive players.

### Per-Position Accuracy

| Position | MAE | Spearman | Mean Actual | Mean Predicted | Notes |
|----------|-----|----------|-------------|----------------|-------|
| GKP | 0.661 | **0.831** | 1.11 | 1.05 | Strongest ranking |
| FWD | 0.990 | **0.811** | 1.46 | 1.57 | Strong |
| MID | 0.943 | **0.760** | 1.28 | 1.17 | Largest group, solid |
| DEF | 1.139 | 0.658 | 1.47 | 1.07 | Weakest — systematic underprediction |

### Ranking Accuracy (top-20 overlap per GW)

Our model identifies 7-10 of the actual top-20 scorers each GW (avg ~7.4/20). ep_next averages ~2.6/20. One outlier week (GW26: 0/20) due to high-variance results.

### Known Weaknesses

1. **DEF underprediction**: Predicts 1.07 avg vs 1.47 actual — misses ~0.4 pts systematically. DEF sub_goals Spearman is -0.018 (random). Clean sheet and defensive bonus prediction is the main gap.
2. **Only 2 seasons of training data**: 33,613 rows. More historical seasons would help rare events and smaller positions.
3. **Feature selection not active**: `feature_selection.py` exists but UI removed. DEFAULT_FEATURES are hand-crafted (40-63 per position), never pruned empirically.
4. **3-GW prediction is a simple sum**: No adjustment for form regression, rotation risk increase, or injury probability over time.

### Hyperparameter Tuning Experiment (Feb 2025)

Tested on `model-improvements` branch — all changes degraded performance. Branch deleted, no changes merged.

**What was tried**:
- Wider hyperparameter grid (max_depth 3-6, learning_rate 0.01-0.1, subsample/colsample 0.7-0.9)
- XGBoost early stopping (validation split from training data, 25-30 rounds patience)
- Sub-model tuning (tune=True instead of fixed params)
- Quantile model tuning (grid search for Q80 models)
- Raised soft caps (GKP 7→8, DEF 8→10, MID 10→12, FWD 10→12)

**Results across 4 rounds of training + evaluation (GW17-25)**:

| Change | MAE Δ | Spearman Δ | Verdict |
|--------|-------|------------|---------|
| Full suite (grid + ES + sub tuning) | +0.159 | -0.019 | Much worse |
| Fixed final model (no ES holdout) | +0.131 | -0.009 | Worse |
| Conservative grid + min tree floor | +0.097 | -0.006 | Worse |
| Soft caps only (no training changes) | +0.000 | +0.000 | Neutral |

**Key findings**:
- Grid search consistently picked overly conservative params (lr=0.01-0.03, depth=3-4) that underfitted recent data
- Early stopping cut sub-models to as few as 4 trees — essentially stumps
- The fixed defaults (150 trees, depth 5, lr 0.1, subsample 0.8) are already a sweet spot for this problem
- Walk-forward CV optimizes for average accuracy across ALL historical splits, but recent-GW performance matters more for FPL
- Soft caps (GKP=7, DEF=8, MID/FWD=10) are rarely hit by actual predictions, so raising them has no effect

**Lesson**: The model architecture and feature engineering are where signal lives, not hyperparameter tuning. Future improvements should focus on better features (especially for DEF) rather than model params.
...
## Build Pipeline & Releases (Feb 2025)

GitHub Actions workflow builds Windows and macOS executables via PyInstaller. Triggers on release creation or manual `workflow_dispatch`. Both jobs run in parallel.

### Files
- **`.github/workflows/build-exe.yml`** — Two parallel jobs: `build-windows` (windows-latest) and `build-macos` (macos-latest). Python 3.12. Installs deps + PyInstaller, builds, zips, uploads to release + as artifact.
- **`fpl-predictor.spec`** — Windows PyInstaller spec. Produces `dist/Gaffer/Gaffer.exe`.
- **`gaffer-mac.spec`** — macOS PyInstaller spec with BUNDLE step. Produces `dist/Gaffer.app` with `Gaffer.icns`.
...
```
This triggers the workflow which builds both zips and attaches them automatically. No manual download/upload needed.

### Manual trigger (no release)
```bash
gh workflow run build-exe.yml
```
Artifacts are downloadable from the Actions tab but not attached to any release.

### Spec file gotchas
- **xgboost.testing**: `collect_all("xgboost")` imports `xgboost.testing` which calls `pytest.importorskip("hypothesis")`. In CI (no hypothesis), this crashes. Fixed with `filter_submodules=lambda name: "testing" not in name`.
- **Hidden imports**: `src.strategy` must be listed. `src.benchmark` was a stale leftover from the old fplxti repo — removed.
...
- **`Gaffer.icns`** — Mac app icon (pitch green circle with white G and pitch markings). Bundled into the .app by the setup script and into the macOS PyInstaller build.
...
## Windows EXE Bugs & Lessons Learned (Feb 2025)

The v1.1.0 Windows EXE shipped completely broken — predictions showed all blank dashes. Took several rounds of debugging to find the root cause. Documenting the full chain here so future sessions don't repeat these mistakes.

### The symptom
On Windows EXE: data fetches OK, models appear to train, but predictions page shows all dashes (no player names, no numbers). On Mac (running from source): everything works perfectly.

### Root cause chain
The real problem was a **cascading failure** triggered by one overly strict validation check:

1. **Bug 60's CSV validation rejected valid empty CSVs** (`data_fetcher.py`). The fix for Bug 60 added `if df.empty or len(df.columns) < 2: raise ValueError(...)` to prevent cache poisoning. But `matches.csv` for future GWs (not yet played) has valid headers (64 columns) and 0 data rows — `df.empty` is True. This raised `ValueError`.

2. **`ValueError` wasn't caught by per-file handlers**. The `fetch_season_data()` per-file exception handlers only caught `requests.RequestException`, not `ValueError`. So the ValueError from step 1 propagated up and killed the entire 2025-2026 season load.

3. **Entire current season skipped**. With 2025-2026 returning `{}`, the pipeline fell back to training on only 2024-2025 data (which ends at GW38 — all historical, no current players).

4. **Predictions were empty/stale**. Models trained on dead data produced no useful predictions for the current GW. The formatting step then had no `web_name` column (since current-season player data was never loaded), causing a secondary crash.

### Why it only happened on Windows
On Mac, the cache directory already had fresh CSVs from previous runs. The `_is_cache_fresh()` check returned True, so `_fetch_csv` never re-validated the data — it just read from cache. On a fresh Windows EXE install (no cache), every file was fetched and validated from scratch, triggering the bug.

### Fixes applied

1. **`data_fetcher.py` — Removed `df.empty` from CSV validation** (commit `0f944c0`):
   ```python
   # Was: if df.empty or len(df.columns) < 2:
   if len(df.columns) < 2:
   ```
   Empty-but-valid CSVs (headers present, 0 rows) are legitimate for future GWs.

2. **`data_fetcher.py` — Catch `ValueError` in all per-file handlers**:
   ```python
   except (requests.RequestException, ValueError) as e:
   ```
   Applied to all 4 exception handlers in `fetch_season_data()` so a single bad file doesn't kill the whole season.

3. **`predict.py` — `web_name` fallback chain** (commit `437c602`, `aed4368`):
   - First tries `web_name` from feature matrix
   - Falls back to bootstrap API cache (`fpl_api_bootstrap.json`)
   - Last resort: uses `player_id` as name so predictions still display
   - Fixed bootstrap cache path to use `_BASE` (not `Path(__file__)` which resolves to PyInstaller temp dir in frozen mode)

4. **`app.py` — Guard `web_name` in search filter** (commit `437c602`):
   ```python
   if search and "web_name" in df.columns:
   ```

### Workflow/release issues

- **`softprops/action-gh-release@v2` needs explicit `tag_name`** (commit `090c213`). Without it, the action fails with "GitHub Releases requires a tag" if the tag gets cleaned up during the workflow. Always pass `tag_name: ${{ github.event.release.tag_name }}`.
- **Artifact upload should be unconditional**. Previously had `if: github.event_name == 'release'` on the artifact upload step, meaning manual `workflow_dispatch` runs produced no downloadable output. Removed the condition.
- **Don't create releases before pushing code**. If you create a release tag pointing at an old commit, the workflow builds from that old commit — not from HEAD. Always push all code changes first, then create the release.

### Key lesson
**Always test the full pipeline on a clean install with no cache.** The Mac development environment masks bugs because cached data bypasses validation. The Windows EXE (or any fresh install) hits every code path from scratch. A "pipeline check" should include `force=True` to bypass all caches.

---

## PyInstaller Frozen-Mode Path Detection (Feb 2025)

### The problem
When running as a PyInstaller EXE (Windows or macOS build), `Path(__file__)` resolves to the **temp extraction directory** (e.g. `C:\Users\...\AppData\Local\Temp\_MEIxxxxx\src\`), not the directory where the EXE lives. Any code that uses `Path(__file__).parent.parent / "output"` or similar will look for files in the temp dir, where `output/`, `models/`, `cache/` etc. don't exist.
...
- `sys.frozen` is only `True` inside a PyInstaller bundle — it does not exist when running from source
- `sys.executable` points to the EXE itself (e.g. `C:\Users\neil\Downloads\Gaffer-Windows\Gaffer.exe`)
- This is safe for Mac/Linux running from source: `getattr(sys, "frozen", False)` returns `False`, so it falls back to the normal `Path(__file__)` approach

### Files that need this pattern
All files that reference filesystem paths for `output/`, `models/`, or `cache/`:

| File | Status | What it resolves |
|------|--------|------------------|
| `src/predict.py` | ✅ Has frozen detection | `OUTPUT_DIR` |
| `src/model.py` | ✅ Has frozen detection | `MODEL_DIR`, `OUTPUT_DIR` |
| `src/data_fetcher.py` | ✅ Has frozen detection | `CACHE_DIR` |
| `src/feature_selection.py` | ✅ Has frozen detection | `OUTPUT_DIR` |
| `src/season_manager.py` | ✅ Fixed Feb 2025 | `OUTPUT_DIR` |
| `src/season_db.py` | ✅ Fixed Feb 2025 | `DB_PATH` |
| `src/app.py` | ✅ Imports `OUTPUT_DIR` from `predict.py` | — |
| `src/strategy.py` | ✅ No filesystem paths | — |
| `src/solver.py` | ✅ No filesystem paths | — |

### How to verify
If a new file is added that reads/writes to `output/`, `models/`, `cache/`, or `*.db`, it **must** include the frozen detection block. Without it:
- Running from source (Mac/Linux dev): works fine
- Running as PyInstaller EXE (Windows/macOS release): **silently fails** — files not found, features broken

### What broke (v1.1.3)
`season_manager.py` and `season_db.py` were missing frozen detection. On the Windows EXE:
- Predictions tab worked (loaded by `predict.py` which had the fix)
- Season → Generate Recommendation failed with "No predictions. Train models first." because `season_manager.py` looked for `predictions.csv` in the temp dir
- Season DB was also created in the temp dir, so season data was lost between runs
